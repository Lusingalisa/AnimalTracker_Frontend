...
import React, { useState, useEffect } from 'react';
import axios from 'axios';
import { FaMapMarkerAlt, FaExclamationTriangle, FaCheck } from 'react-icons/fa';
...
const GeofenceAlerts = () => {
  ...
  useEffect(() => {
    const fetchAlerts = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await axios.get(`${import.meta.env.VITE_API_URL}/api/geofences/alerts`, {
          params: { status: 'active' },
          headers: { Authorization: `Bearer ${token}` },
        });
        setAlerts(response.data);
  ...
  if (loading) return <div className="text-center my-4">Loading alerts...</div>;
  if (error) return <div className="alert alert-danger">{error}</div>;
